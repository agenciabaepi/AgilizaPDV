...
/** Lista NF-e (Postgres) — espelha `nfe.service.listNfe` para terminais remotos. */
r.get('/', async (req, res) => {
  ...
  if (search) {
    const term = `%${search}%`
    sql += ` AND (n.numero_nfe::text ILIKE $${i} OR v.numero::text ILIKE $${i} OR COALESCE(c.nome, '') ILIKE $${i})`
    params.push(term)
    i++
  }
  sql += ` ORDER BY n.created_at DESC LIMIT $${i}`
  params.push(limit)
  ...
  try {
    ...
    res.json(
      rows.map((row) => ({
        ...
        numero_nfe: Number(row.numero_nfe),
        status: row.status,
        ...
        nfe_created_at: row.nfe_created_at != null ? String(row.nfe_created_at) : null,
        venda_total: Number(row.venda_total),
        cliente_nome: row.cliente_nome ?? null
      }))
    )
  } catch (e) {
    res.status(500).json({ error: e instanceof Error ? e.message : 'Erro ao listar NF-e.' })
  }
})
