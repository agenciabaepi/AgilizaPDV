...
function sanitizePplbText(value: string): string {
...
function pushTextCommand(commands: string[], x: number, y: number, role: string, text: string): void {
  ...
  commands.push(`A${x},${y},0,${font},${xmul},${ymul},N,"${sanitizePplbText(text)}"`)
}
...
function pushBarcodeCommand(commands: string[], x: number, y: number, height: number, value: string): void {
  ...
  if (ean13Data) {
    // E30 = EAN-13 em PPLB; enviamos 12 dígitos para checksum automático da impressora.
    // "N" desativa HRI para posicionarmos o texto humano no layout.
  ...
  if (!cleaned) return
  // Fallback para códigos não EAN-13.
...
export class PplbGenerator implements CommandGenerator {
