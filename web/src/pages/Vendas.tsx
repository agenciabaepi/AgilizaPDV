...
import type { VendaComNfce, StatusNfce, VendaDetalhes } from '../vite-env'
import { PageTitle, Button, ConfirmDialog, Dialog } from '../components/ui'
import { Printer, Trash2, Calendar, Receipt, DollarSign, CheckCircle, XCircle, FileCheck, FileText, Eye, ExternalLink, Store, ShoppingBag } from 'lucide-react'
...
type OrigemVenda = 'todas' | 'pdv' | 'online'
...
const ORIGEM_ABAS: { id: OrigemVenda; label: string; icon: React.ReactNode }[] = [
  { id: 'todas', label: 'Todas', icon: <Receipt size={16} /> },
  { id: 'pdv', label: 'PDV', icon: <Store size={16} /> },
  { id: 'online', label: 'Online', icon: <ShoppingBag size={16} /> },
]
...
export function Vendas() {
  ...
  const [origem, setOrigem] = useState<OrigemVenda>('todas')
  ...
  const [detalhesVendaLoading, setDetalhesVendaLoading] = useState(false)
  type NfceViewData = {
  ...
  useEffect(() => {
    if (!visualizarVendaId) {
      ...
      setDetalhesVendaLoading(false)
      return
    }
    setDetalhesVenda(null)
    setDetalhesVendaLoading(true)
    window.electronAPI.cupom
      .getDetalhes(visualizarVendaId)
      .then((det) => {
        if (det) {
          setDetalhesVenda(det as VendaDetalhes)
        }
      })
      .catch(() => {
        setDetalhesVenda(null)
      })
      .finally(() => setDetalhesVendaLoading(false))
  }, [visualizarVendaId])
  ...
  const totalNoCaixa = vendasFiltradas.filter((v) => !isPrazo(v)).reduce((acc, v) => acc + v.total, 0)
  const totalPrazo = vendasFiltradas.filter((v) => isPrazo(v)).reduce((acc, v) => acc + v.total, 0)
  ...
  return (
    <Layout>
      ...
      <div className="vendas-cards-resumo">
        <div className="vendas-card-resumo vendas-card-resumo--vendas">
          ...
          <div className="vendas-card-resumo__content">
            ...
            <span className="vendas-card-resumo__value">{vendasFiltradas.length}</span>
          </div>
        </div>
        ...
        <div className="vendas-abas">
          {ORIGEM_ABAS.map((aba) => (
            <button
              key={aba.id}
              type="button"
              className={`btn btn--secondary btn--sm ${origem === aba.id ? 'vendas-aba-ativa' : ''}`}
              onClick={() => setOrigem(aba.id)}
            >
              {aba.icon}
              {aba.label}
            </button>
      ...
      <div className="vendas-table-wrap">
        <h3 className="vendas-lista-title">
          {origem === 'online' ? 'Vendas online' : origem === 'pdv' ? 'Vendas do PDV' : 'Cupons'}
        </h3>
        <p className="vendas-lista-subtitle" style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-muted)', marginTop: 4, marginBottom: 12 }}>
          {origem === 'online'
            ? 'Vendas finalizadas pela loja online. Use "Detalhes" para ver pedido, cliente e endereço.'
                  ...
                  <tr key={v.id} className="vendas-table__row">
                    ...
                    <td className="vendas-table__situacao">
                      <div className="vendas-situacao-badges">
                        ...
                        {v.venda_online === 1 && (
                          <span className="vendas-badge-online" title="Venda finalizada pela loja online">
                            Online
                          </span>
                        )}
                      </div>
            ...
            <div className="vendas-detalhes-header">
              <div>
                ...
                <div className="vendas-detalhes-info">
                  ...
                  {detalhesVenda.cupom_empresa?.vendedor_nome && (
                    <span>Vendedor: {detalhesVenda.cupom_empresa.vendedor_nome}</span>
                  )}
                </div>
              </div>
              ...
              <div className="vendas-detalhes-pedido-grid">
                <div className="vendas-detalhes-card">
                  ...
                  <dl className="vendas-detalhes-dl">
                    <div>
                      <dt>Código</dt>
                      <dd>#{detalhesVenda.pedido_online.id.slice(0, 8).toUpperCase()}</dd>
                    </div>
                    <div>
                      <dt>Status</dt>
                      <dd>{labelPedidoStatus(detalhesVenda.pedido_online.status)}</dd>
                    </div>
                    <div>
                      <dt>Data do pedido</dt>
                      <dd>
                        {new Date(detalhesVenda.pedido_online.created_at).toLocaleString('pt-BR', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric',
                          hour: '2-digit',
                    ...
                    <div>
                      <dt>Entrega</dt>
                      <dd>
                        {detalhesVenda.pedido_online.forma_entrega === 'entrega'
                          ? 'Delivery'
                          : detalhesVenda.pedido_online.forma_entrega === 'retirada'
                            ? 'Retirada na loja'
                            : '—'}
                      </dd>
                    </div>
                    {detalhesVenda.pedido_online.forma_pagamento && (
                      <div>
                        <dt>Pagamento</dt>
                        <dd>
                          {FORMA_PAGAMENTO_LABEL[detalhesVenda.pedido_online.forma_pagamento] ??
                            detalhesVenda.pedido_online.forma_pagamento}
                          {detalhesVenda.pedido_online.pagamento_status
                            ? ` · ${PAGAMENTO_STATUS_LABEL[detalhesVenda.pedido_online.pagamento_status] ?? detalhesVenda.pedido_online.pagamento_status}`
                            : ''}
                        </dd>
                      </div>
                    )}
                    {detalhesVenda.pedido_online.cupom_codigo && (
                      <div>
                        <dt>Cupom</dt>
                        <dd>{detalhesVenda.pedido_online.cupom_codigo}</dd>
                      </div>
                    )}
                    {detalhesVenda.pedido_online.observacoes && (
                      <div>
                        <dt>Observações</dt>
                        <dd>{detalhesVenda.pedido_online.observacoes}</dd>
                      </div>
                    )}
                  </dl>
                </div>
                ...
                <div className="vendas-detalhes-card">
                  ...
                  <dl className="vendas-detalhes-dl">
                    <div>
                      <dt>Nome</dt>
                      <dd>{detalhesVenda.pedido_online.cliente_nome ?? '—'}</dd>
                    </div>
                    <div>
                      <dt>E-mail</dt>
                      <dd>{detalhesVenda.pedido_online.cliente_email ?? '—'}</dd>
                    </div>
                    <div>
                      <dt>Telefone</dt>
                      <dd>{detalhesVenda.pedido_online.cliente_telefone ?? '—'}</dd>
                    </div>
                  </dl>
                </div>
                ...
                <div className="vendas-detalhes-card vendas-detalhes-card--endereco">
                  <h4>Endereço</h4>
                  {detalhesVenda.pedido_online.endereco_entrega ? (
                    <dl className="vendas-detalhes-dl">
                      {detalhesVenda.pedido_online.cep_destino && (
                        <div>
                          <dt>CEP</dt>
                          <dd>{detalhesVenda.pedido_online.cep_destino}</dd>
                        </div>
                      )}
                      <div>
                        <dt>Endereço completo</dt>
                        <dd>{detalhesVenda.pedido_online.endereco_entrega}</dd>
                      </div>
                    </dl>
                  ) : (
                  ...
                  {(detalhesVenda.pedido_online.valor_frete != null && detalhesVenda.pedido_online.valor_frete > 0) ||
                  (detalhesVenda.pedido_online.valor_desconto != null && detalhesVenda.pedido_online.valor_desconto > 0) ||
                  (detalhesVenda.pedido_online.cashback_usado != null && detalhesVenda.pedido_online.cashback_usado > 0) ? (
                    <dl className="vendas-detalhes-dl vendas-detalhes-dl--resumo">
                      {detalhesVenda.pedido_online.valor_frete != null && detalhesVenda.pedido_online.valor_frete > 0 && (
                        <div>
                          <dt>Frete</dt>
                          <dd>R$ {detalhesVenda.pedido_online.valor_frete.toFixed(2)}</dd>
                        </div>
                      )}
                      {detalhesVenda.pedido_online.valor_desconto != null && detalhesVenda.pedido_online.valor_desconto > 0 && (
                        <div>
                          <dt>Desconto</dt>
                          <dd>R$ {detalhesVenda.pedido_online.valor_desconto.toFixed(2)}</dd>
                        </div>
                      )}
                      {detalhesVenda.pedido_online.cashback_usado != null && detalhesVenda.pedido_online.cashback_usado > 0 && (
                        <div>
                          <dt>Cashback usado</dt>
                          <dd>R$ {detalhesVenda.pedido_online.cashback_usado.toFixed(2)}</dd>
                        </div>
                      )}
                    </dl>
                  ) : null}
