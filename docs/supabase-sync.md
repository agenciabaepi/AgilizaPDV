...
- **Digest de cadastros (web ↔ app):** a cada ~1 min (app SQLite local) e ~55 s (Postgres no store-server) o sistema compara **contagem + timestamps máximos** nas tabelas espelhadas principais (`empresas`, `usuarios`, `empresas_config`, `categorias`, `marcas`, `produtos`, `clientes`, `fornecedores`) entre o banco local e o Supabase. Se divergir, executa **push (outbox) + pull do espelho** e notifica a UI. Isto **não** substitui comparação campo a campo em todas as linhas, mas apanha a maior parte dos dessincs quando web e PDV são usados em paralelo. Reconciliação manual: IPC `sync:mirrorReconcile` / `POST /sync/mirror-reconcile` no store-server.
